...
def posisiCallback(posisi_message):
    ...
    twist = Twist()
    speed_x = 0.1 + pid_jarak
    speed_yaw = pid_yaw

    if speed_x > 0.2:
        speed_x = 0.2

    if jarak_tempuh < 0.01:     # 1cm
        speed_x = 0.0
        waypoint += 1
        if waypoint > 4:
            waypoint = 1

    if error_yaw > 45 or error_yaw < -45 :      # si robot di suruh muter dulu(kecepatan linear.x nya di 0 kan), kalau error_yaw lebih besar dari 10 derajar atau lebih kecil -10 derajar
        speed_x = 0.0
